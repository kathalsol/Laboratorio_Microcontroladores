		...
		switch(state){
			...
			case(start_wash): 
				if(interrupt_1 == 1){ // Interrupt 1 = Baja
					interrupt_1 =0;
					green_light_charge(time_delay); // Indica que es un lavado con carga baja
					...
					interrupt_2 =0;
					yellow_light_charge(time_delay); // Indica que es un lavado con carga media
					//next_state = suministro_agua;
					...
					interrupt_3 =0;
					red_light_charge(time_delay); // Indica que es un lavado con carga media
					//next_state = suministro_agua;
			...
			case(lavar):
				yellow_light(time_delay);
				//next_state = enjuagar;
			break;

			case(centrifugar):
				blue_light(time_delay);
				//next_state = checking;
			break;
